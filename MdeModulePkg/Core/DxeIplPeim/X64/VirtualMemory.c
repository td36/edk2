...
/**
  This function modifies the page MapAttribute for the memory region specified by BaseAddress and
  Length from their current attributes to the attributes specified by MapAttribute.
  ...
  )
{
  ...
  Status              = PageTableMap (&PageTableBase, PagingMode, NULL, &PageTableBufferSize, BaseAddress, Length, MapAttribute, MapMask);
  DEBUG ((DEBUG_INFO, "ConvertMemoryPageAttributes: 0x%x bytes needed for page table\n", PageTableBufferSize));
  if (Status == RETURN_BUFFER_TOO_SMALL) {
    PageTableBuffer = AllocatePageTableMemory (EFI_SIZE_TO_PAGES (PageTableBufferSize));
    ASSERT (PageTableBuffer != NULL);
    Status = PageTableMap (&PageTableBase, PagingMode, PageTableBuffer, &PageTableBufferSize, BaseAddress, Length, MapAttribute, MapMask);
  }
  ...
  )
{
  ...
  do {
    ...
    //
    ConvertMemoryPageAttributes (PageTableBase, PagingMode, Address, PoolSize, &MapAttribute, &MapMask);
    Pool = Pool->NextPool;
  } while (Pool != HeadPool);
  ...
  )
{
  ...
  MapMask.Uint64              = ~0ull;
  PageTableBufferSize         = 0;
  Status                      = PageTableMap (&PageTable, PagingMode, NULL, &PageTableBufferSize, 0, LShiftU64 (1, PhysicalAddressBits), &MapAttribute, &MapMask);
  ASSERT (Status == RETURN_BUFFER_TOO_SMALL);
  DEBUG ((DEBUG_INFO, "DxeIpl: 0x%x bytes needed for page table\n", PageTableBufferSize));
  PageTableBuffer = AllocatePageTableMemory (EFI_SIZE_TO_PAGES (PageTableBufferSize));
  ASSERT (PageTableBuffer != NULL);
  Status = PageTableMap (&PageTable, PagingMode, PageTableBuffer, &PageTableBufferSize, 0, LShiftU64 (1, PhysicalAddressBits), &MapAttribute, &MapMask);
  ASSERT_RETURN_ERROR (Status);
  ASSERT (PageTableBufferSize == 0);

  if (!Is32BitPageTable && (GhcbSize > 0) && (GhcbBase > 0) && (AddressEncMask != 0)) {
    //
    ...
    ConvertMemoryPageAttributes (PageTable, PagingMode, GhcbBase4K, SIZE_4KB, &MapAttribute, &MapMask);
  }
  ...
  if (PcdGetBool (PcdSetNxForStack)) {
    ...
    MapMask.Uint64       = 0;
    MapMask.Bits.Nx      = 1;
    ConvertMemoryPageAttributes (PageTable, PagingMode, StackBase, StackSize, &MapAttribute, &MapMask);
  }
  ...
  if (IsNullDetectionEnabled ()) {
    ...
    //
    ConvertMemoryPageAttributes (PageTable, PagingMode, 0, SIZE_4KB, &MapAttribute, &MapMask);
  }
  ...
  if (PcdGetBool (PcdCpuStackGuard)) {
    ...
    //
    ConvertMemoryPageAttributes (PageTable, PagingMode, StackBase, SIZE_4KB, &MapAttribute, &MapMask);
  }
