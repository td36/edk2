...
class UnitTestSupport():
    ...
    UTPkgsToBuild   = ["UefiCpuPkg", "MdeModulePkg"]
    '''
    UnitTestModule = {}
    UTPkgsToBuild  = []

    def UpdateUnitTestConfig(self, args):
        '''Update UnitTestModule, PackagesSupported and UTPkgsToBuild in UnitTestSupport class'''
        ...
        try:
            UTPkgsToBuild = ','.join(args.PackagesForUT).split(',')
            if UTPkgsToBuild:
                UnitTestSupport.UTPkgsToBuild = UTPkgsToBuild
                print('UTPkgsToBuild is {}'.format(UnitTestSupport.UTPkgsToBuild))
        except:
            pass

    def BuildUnitTest(self):
        UTPkgsToBuild = UnitTestSupport.UTPkgsToBuild
        if not UTPkgsToBuild:
            return
        self.VirtualDrive = os.path.join(self.env.GetValue("BUILD_OUTPUT_BASE"), "VirtualDrive")
        # OvmfPkg should have been built in BUILD process.
        BuiltDsc = ['OvmfPkg']
        for package in UTPkgsToBuild:
            print(package)
    ...
    def WriteEfiToStartup(self):
        ...
        if UnitTestSupport.UTPkgsToBuild and (self.env.GetValue("MAKE_STARTUP_NSH").upper() == "TRUE"):
    ...
    def CheckBootLog(self):
        ...
        #
        if not UnitTestSupport.UTPkgsToBuild:
            return 0
...
class UnitTestPlatformBuilder(PlatformBuilder):
    ...
    def PlatformPostBuild(self):
        ...
        UnitTestUnitTestConfig.BuildUnitTest()
        UnitTestUnitTestConfig.WriteEfiToStartup()
        return 0
