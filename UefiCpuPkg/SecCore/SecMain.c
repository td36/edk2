  ...
  )
{
  ...
  SEC_IDT_TABLE_OR_FS_PAGE  StorageInStack;
  UINT32                    Index;
  UINT32                    PeiStackSize;
  EFI_STATUS                Status;
  IA32_CR4                  Cr4;
  ...
  // |                   |
  // |-------------------|---->  TempRamBase
  StorageInStack.PeiService = 0;
  if (FixedPcdGet32 (PcdFredEnable) && IsFredSupported ()) {
    Cr4.UintN     = AsmReadCr4 ();
    Cr4.Bits.FRED = 1;
    AsmWriteCr4 (Cr4.UintN);
    AsmWriteMsr64 (MSR_IA32_FS_BASE, (UINT64)(UINTN)&StorageInStack);
  } else {
    for (Index = 0; Index < SEC_IDT_ENTRY_COUNT; Index++) {
      ZeroMem ((VOID *)&StorageInStack.IdtTableOrFsPage.IdtTable[Index], sizeof (IA32_IDT_GATE_DESCRIPTOR));
      CopyMem ((VOID *)&StorageInStack.IdtTableOrFsPage.IdtTable[Index], (VOID *)&mIdtEntryTemplate, sizeof (UINT64));
    }

    IdtDescriptor.Base  = (UINTN)&StorageInStack.IdtTableOrFsPage.IdtTable;
    IdtDescriptor.Limit = (UINT16)(sizeof (StorageInStack.IdtTableOrFsPage.IdtTable) - 1);
