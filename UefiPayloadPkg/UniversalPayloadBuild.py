...
def BuildUniversalPayload(Args, MacroList):
    ...
    #
    remove_section = '"%s" -I elf32-i386 -O elf32-i386 --remove-section .upld_info --remove-section .upld.uefi_fv %s'%(LlvmObjcopyPath, EntryOutputDir)
    add_section    = '"%s" -I elf32-i386 -O elf32-i386 --add-section .upld_info=%s --add-section .upld.uefi_fv=%s %s'%(LlvmObjcopyPath, UpldInfoFile, FvOutputDir, EntryOutputDir)
    set_section    = '"%s" -I elf32-i386 -O elf32-i386 --set-section-alignment .upld.upld_info=16 --set-section-alignment .upld.uefi_fv=16 %s'%(LlvmObjcopyPath, EntryOutputDir)
