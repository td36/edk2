  ...
  )
{
  ...
  {
    DEBUG ((DEBUG_INFO, "return there\n"));
    return FALSE;
  }
  DEBUG ((DEBUG_INFO, "Succuss decompress\n"));
  //
  // Support little-endian only
  //
  if (Elf32Hdr->e_ident[EI_DATA] != ELFDATA2LSB) {
    DEBUG ((DEBUG_INFO, "return there1\n"));
    return FALSE;
  }
  ...
  if (Elf32Hdr->e_ident[EI_CLASS] == ELFCLASS64) {
    ...
    DEBUG ((DEBUG_INFO, "return there2\n"));
    return FALSE;
  }
  ...
  if (Elf64Hdr != NULL) {
    ...
    if (Elf64Hdr->e_machine != EM_X86_64) {
      DEBUG ((DEBUG_INFO, "return there3\n"));
      return FALSE;
    }
    ...
    if ((Elf64Hdr->e_type != ET_EXEC) && (Elf64Hdr->e_type != ET_DYN)) {
      DEBUG ((DEBUG_INFO, "return there4\n"));
      return FALSE;
    }
    ...
    if (Elf64Hdr->e_version != EV_CURRENT) {
      DEBUG ((DEBUG_INFO, "return there5\n"));
      return FALSE;
    }
  } else {
    //
    ...
    if (Elf32Hdr->e_machine != EM_386) {
      DEBUG ((DEBUG_INFO, "return there6\n"));
      return FALSE;
    }
    ...
    if ((Elf32Hdr->e_type != ET_EXEC) && (Elf32Hdr->e_type != ET_DYN)) {
      DEBUG ((DEBUG_INFO, "return there7\n"));
      return FALSE;
    }
    ...
    if (Elf32Hdr->e_version != EV_CURRENT) {
      DEBUG ((DEBUG_INFO, "return there8\n"));
      return FALSE;
    }
  }
